...
export function ContentRenderer({ content, type, label }: ContentRendererProps) {
  ...
  const apiBase = (import.meta as any).env?.VITE_API_BASE_URL || 'http://localhost:8100';
  ...
  } else if (type === 'image' || label.toLowerCase().includes('image') || label.toLowerCase().includes('chart') || label.toLowerCase().includes('figure')) {
    // content 为 /api/documents/.../images/... 路径
    const imgSrc = content.startsWith('http') ? content : `${apiBase}${content}`;
    return (
