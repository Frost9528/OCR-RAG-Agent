...
export function DocumentPreview() {
  ...
  const loadVisualizations = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE}/api/documents/${docId}/visualizations`);
      if (response.ok) {
                        ...
                        <div key={file.path} className="bg-card/50 rounded-lg p-4 border border-border/50">
                          ...
                          {file.type.startsWith('image/') ? (
                            <img
                              ...
                              onClick={() => setEnlargedImage(`${API_BASE}${file.path}`)}
                            />
                          ) : (
                            <div className="text-sm text-foreground">
                              PDF 文件 - <a href={`${API_BASE}${file.path}`} target="_blank" rel="noopener noreferrer" className="text-accent hover:underline">查看</a>
                            </div>
                          )}
                        </div>
                      ))}
