...
"""
            ...
            # dashscope Generation.call() 是同步阻塞调用，直接在 async 中迭代会占据
            # event loop 导致无法真正流式推送。改为：在独立线程中运行，通过
            # asyncio.Queue 把每个 chunk 传回 event loop，实现真正的逐块推送。
            import threading
            import asyncio
                        ...
                        if resp.status_code == 200:
                            ...
                            if hasattr(resp, 'output'):
                                if hasattr(resp.output, 'choices') and len(resp.output.choices) > 0:
                                    ...
                                    chunk = resp.output.text
                                elif isinstance(resp.output, dict):
                                    chunk = resp.output.get('text', '')
                            if chunk:
                                asyncio.run_coroutine_threadsafe(queue.put(chunk), loop)
                        else:
                            logger.error(f"Stream error: {resp.code} - {resp.message}")
                            break
                except Exception as e:
                    logger.error(f"Stream thread error: {e}")
                finally:
                    asyncio.run_coroutine_threadsafe(queue.put(None), loop)  # 结束哨兵
