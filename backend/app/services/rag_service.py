...
class RAGService:
    ...
    def format_citations(self, results: List[Dict[str, Any]]) -> List[Citation]:
        ...
        for i, result in enumerate(results):
            ...
            # 图片路径：优先使用 content（ocr_service 已设置正确的 API 路径）
            if block_type == "image":
                if content.startswith('/api/'):
                    image_path = content
                elif bbox and len(bbox) == 4:
                    # Fallback：遍历常见 label 前缀
                    for prefix in ['image', 'chart', 'figure', block_label.lower()]:
                        image_filename = f"img_in_{prefix}_box_{bbox[0]}_{bbox[1]}_{bbox[2]}_{bbox[3]}.jpg"
                        image_path = f"/api/documents/{doc_id}/images/{image_filename}"
                        break  # 使用 block_label 作为 prefix，直接取第一个
