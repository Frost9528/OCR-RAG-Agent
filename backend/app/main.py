        ...
        for file_index, json_file in enumerate(json_files):
            ...
            for block in blocks:
                ...
                if 'image' in label or 'figure' in label or 'chart' in label:
                    # 优先使用 block_content（ocr_service 已设置正确的 API 路径）
                    block_content = block.get('block_content', '')
                    if block_content.startswith('/api/'):
                        block['image_path'] = block_content
                    else:
                        # Fallback：遍历常见 label 前缀查找图片文件
                        bbox = block.get('block_bbox', [])
                        if bbox and len(bbox) >= 4:
                            for prefix in ['image', 'chart', 'figure']:
                                img_pattern = f"img_in_{prefix}_box_{bbox[0]}_{bbox[1]}_{bbox[2]}_{bbox[3]}.jpg"
                                if (imgs_dir / img_pattern).exists():
...
async def get_document_visualizations(doc_id: str):
    ...
    try:
        ...
        # 查找布局检测结果图（支持 png/jpg）
        layout_det_files = [
            f for ext in ("*.png", "*.jpg", "*.jpeg")
            for f in doc_dir.glob(f"*layout_det_res*") if f.suffix.lower() in (".png", ".jpg", ".jpeg")
        ]
        seen = set()
        ...
        # 查找阅读顺序图（支持 png/jpg）
        layout_order_files = [
            f for f in doc_dir.glob("*layout_order_res*")
